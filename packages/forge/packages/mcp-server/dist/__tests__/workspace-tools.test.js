...
(0, vitest_1.describe)('FORGE_WORKSPACE_PATH env var resolution', () => {
    ...
    (0, vitest_1.it)('startMcpServer uses FORGE_WORKSPACE_PATH when set', async () => {
        const ForgeCoreMock = vitest_1.vi.fn().mockImplementation(() => ({
            workspaceList: vitest_1.vi.fn().mockResolvedValue([]),
        }));
        vitest_1.vi.doMock('@forge/core', async (importOriginal) => {
            const actual = await importOriginal();
            return { ...actual, ForgeCore: ForgeCoreMock };
        });
        const tmpDir = await fs_1.promises.mkdtemp(path_1.default.join(os_1.default.tmpdir(), 'forge-env-test-'));
        ...
        (0, vitest_1.expect)(resolved).toBe(tmpDir);
        (0, vitest_1.expect)(resolved).not.toBe(process.cwd());
        await fs_1.promises.rm(tmpDir, { recursive: true, force: true });
        vitest_1.vi.doUnmock('@forge/core');
    });
    (0, vitest_1.it)('falls back to process.cwd() when FORGE_WORKSPACE_PATH is not set', () => {
        delete process.env.FORGE_WORKSPACE_PATH;
        const resolved = process.env.FORGE_WORKSPACE_PATH ?? process.cwd();
        (0, vitest_1.expect)(resolved).toBe(process.cwd());
    });
});
