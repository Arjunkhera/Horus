...
function resolveClonePath(opts: {
  ...
  hostMountPath?: string;    // globalConfig.workspace.host_workspaces_path (Docker only)
}): string {
  const hostMount = opts.hostMountPath ?? opts.mountPath;
  let effectiveRoot = path.resolve(opts.workspacePath ?? opts.workspaceRoot);
  // Translate host workspacePath → container path when running in Docker
  if (hostMount !== opts.mountPath && effectiveRoot.startsWith(hostMount + path.sep)) {
...
describe('repoClone path selection', () => {
  ...
  describe('host-path translation (Docker regression)', () => {
    const dockerMountPath = '/data/workspaces';
    const dockerHostMount = '/Users/arkhera/.horus/data/workspaces';

    it('translates host workspacePath to container path (the regression)', () => {
      ...
      expect(result).toBe('/data/workspaces/sdlc-default-cb06024d-e31e-4004-9a2a-e94381121e0a/my-repo');
    });

    it('still falls back to mount root when workspacePath is outside both mount and hostMount', () => {
      const result = resolveClonePath({
        repoName,
        workspacePath: '/some/other/path',
        workspaceRoot: '/app',
    ...
    it('accepts container path directly without translation', () => {
      const result = resolveClonePath({
        repoName,
      ...
      expect(result).toBe('/data/workspaces/sdlc-default-ws-xyz/my-repo');
    });

    it('does not translate when hostMountPath equals mountPath (no-op for non-Docker)', () => {
      const result = resolveClonePath({
        repoName,
      ...
      expect(result).toBe('/data/workspaces/sdlc-default-ws-abc/my-repo');
    });
  });
});
