...
// ── forge.yaml generation ───────────────────────────────────────────────────

/**
 * Marker placed in generated forge.yaml. Setup regenerates the file only while
 * this marker is present; removing it opts the file out of CLI management.
 */
export const FORGE_CONFIG_MANAGED_MARKER = 'Managed by @arkhera30/cli';

/**
 * Generate ~/Horus/data/config/forge.yaml — the config for the Forge engine
 * embedded in horus-ui (`[forge-local-mcp]`).
 *
 * The critical job is the registry list. In connected mode we point both the
 * `local` and `global` registries at the control-plane Forge registry
 * (`${control_plane_url}/api/v1/forge`), authenticated via TOKEN_PROVIDER_CONFIG.
 * Both names are set deliberately so @forge/core's `ensureDefaultRegistries`
 * keeps them and does NOT re-inject its hardcoded defaults — `http://localhost:8744`
 * (no registry-service exists in the alpha topology) and a long-decommissioned
 * CloudFront host. Without this, every workspace-config artifact fetch fails and
 * `forge_workspace_create` produces an unusable workspace.
 *
 * In local-only mode (no control plane) there is no remote registry; we still
 * pin both names to the default local URL so the dead CloudFront default is
 * never used (it fails fast rather than hanging on a dead host).
 *
 * Workspace/repo paths mirror the container mounts so workspaces materialize on
 * the bind-mounted `/horus/workspaces` volume, and `host_*` paths are emitted so
 * Claude Code on the host can resolve session/workspace files.
 */
export function generateForgeConfig(config: Config): string {
  const dataDir = resolvePath(config.data_dir);
  const cp = (config.control_plane_url ?? '').trim().replace(/\/$/, '');
  const registryUrl = cp ? `${cp}/api/v1/forge` : 'http://localhost:8744';
  const registries = cp
    ? [
        { type: 'http', name: 'local', url: registryUrl, writable: true, tokenEnv: 'TOKEN_PROVIDER_CONFIG' },
        { type: 'http', name: 'global', url: registryUrl, writable: false, tokenEnv: 'TOKEN_PROVIDER_CONFIG' },
      ]
    : [
        { type: 'http', name: 'local', url: registryUrl, writable: true },
        { type: 'http', name: 'global', url: registryUrl, writable: false },
      ];

  const forgeConfig = {
    registries,
    workspace: {
      mount_path: '/horus/workspaces',
      default_config: 'sdlc-default',
      store_path: '/horus/config/workspaces.json',
      sessions_path: '/horus/config/sessions.json',
      managed_repos_path: '/horus/repos',
      sessions_root: '/horus/sessions',
      host_workspaces_path: pathJoin(dataDir, 'workspaces'),
      host_managed_repos_path: pathJoin(dataDir, 'repos'),
    },
    mcp_endpoints: {
      anvil: { url: 'http://anvil:8100', transport: 'http' },
    },
    repos: {
      scan_paths: ['/horus/repos'],
      index_path: '/horus/config/repos.json',
    },
  };

  const header = [
    '# ─────────────────────────────────────────────────────────────────────────────',
    '# Horus — Generated Forge engine config (forge.yaml)',
    `# ${FORGE_CONFIG_MANAGED_MARKER}. Regenerated by \`horus setup\`.`,
    '#',
    '# Points the embedded Forge engine (inside horus-ui) at the control-plane',
    '# registry. Without it, @forge/core falls back to dead default registries',
    '# and every workspace artifact fetch fails. Remove the managed marker above',
    '# to hand-manage this file — setup will then leave it untouched.',
    '# ─────────────────────────────────────────────────────────────────────────────',
    '',
  ].join('\n');

  return header + stringifyYaml(forgeConfig);
}

/**
 * Write ~/Horus/data/config/forge.yaml. Idempotent and non-destructive:
 * (re)generates only when the file is absent or still carries the managed
 * marker. A user-edited file (marker removed) is left untouched.
 *
 * Returns whether the file was written and its absolute path.
 */
export function writeForgeConfigFile(config: Config): { written: boolean; path: string } {
  const dataDir = resolvePath(config.data_dir);
  const configDir = pathJoin(dataDir, 'config');
  const forgePath = pathJoin(configDir, 'forge.yaml');
  if (existsSync(forgePath)) {
    const existing = readFileSync(forgePath, 'utf-8');
    if (!existing.includes(FORGE_CONFIG_MANAGED_MARKER)) {
      return { written: false, path: forgePath };
    }
  }
  mkdirSync(configDir, { recursive: true });
  writeFileSync(forgePath, generateForgeConfig(config), 'utf-8');
  return { written: true, path: forgePath };
}
